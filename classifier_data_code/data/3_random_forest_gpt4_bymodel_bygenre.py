...
for domain in df["domain"].unique():
    ...
    for model_name in domain_models:
        ...
        for i in range(100):
            ...
            size = min(len(auth_df), len(synth_df), 1000)

            auth_sample = auth_df.sample(size)
            synth_sample = synth_df.sample(size)

            sample_df = pd.concat([auth_sample, synth_sample]).reset_index(drop=True)
            ...
            file_path = f"{test_dataset}/matrix/{test_dataset}_{domain}_{model_name}_random_forest_final.csv"
            ...
            run_info_df.to_csv(f'{test_dataset}/{test_dataset}_matrix_1000_classifier_log.csv', mode='a', index=False, header=False)
