...
for domain in test_df["domain"].unique():
    ...
    domain_models = test_df[(test_df["domain"] == domain) & (test_df["model"] != "human")]["model"].unique()

    for model_name in domain_models:
        print(f"domain = {domain}, model = {model_name}")

        for i in range(100):
            train = []
            test = []

            # 10 authors: 80 auth, 80 synth, 160 total per author, 1600 total train
            for author in train_df["author"].unique():
                train_auth = train_df[(train_df["author"] == author) & (train_df["category"] == "authentic")]
                train_auth_sample = train_auth.sample(min(len(train_auth), 80))

                train_synth = train_df[(train_df["author"] == author) & (train_df["category"] == "synthetic")]
                train_synth_sample = train_synth.sample(min(len(train_synth), 80))

                train.extend([train_auth_sample, train_synth_sample])

            train_sample_df = pd.concat(train, ignore_index=True)

            # 200 synthetic, 200 authentic, total test size 400 (20% of 2000)
            test_auth_df = test_df[(test_df["domain"] == domain) & (test_df["label"] == "authentic")]
            test_synth_df = test_df[(test_df["domain"] == domain) & (test_df["model"] == model_name) & (test_df["label"] == "synthetic")]

            size = min(len(test_auth_df), len(test_synth_df), 200)

            test_auth_sample = test_auth_df.sample(size)
            test_synth_sample = test_synth_df.sample(size)

            test_sample_df = pd.concat([test_auth_sample, test_synth_sample]).reset_index(drop=True)
            ...
            file_path = f"{test_dataset}/HDSR_train/matrix_1000/{test_dataset}_HDSR_train_{domain}_{model_name}_random_forest_final.csv"
            ...
            run_info = { "user": "Zeina Daboul",
                        ...
                        "testing_models": model_name,
                        "training_domains": domain,
                        "testing_domains": domain,
                        "accuracy": accuracy,
            ...
            run_info_df.to_csv(f'{test_dataset}/{test_dataset}_HDSR_train_matrix_1000_classifier_log.csv', mode='a', index=False, header=False)
            print(f"Run {i} complete. Starting next run.")

        print(f"Model {model_name} complete. Starting next run.")
    print(f"Domain {domain} complete. Starting next run.")
