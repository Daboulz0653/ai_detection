...
df = pd.read_csv(f"{test_dataset}/{test_dataset}_final_feature_matrix.csv")
...
for i in range(100):
    ...
    for domain in df["domain"].unique():
        ...
        domain_models = df[(df["domain"] == domain) & (df["model"] != "human")]["model"].unique()

        test_auth = df[(df["domain"] == domain) & (df["label"] == "authentic")]
        size = min(len(test_auth), 100 * len(domain_models))
        test_auth_sample = test_auth.sample(size)
        sample_list.extend([test_auth_sample])


        per_model_size = round(size/len(domain_models))
        for model in df[(df["domain"] == domain) & (df["model"] != "human")]["model"].unique():
            test_synth = df[(df["domain"] == domain) & (df["model"] == model) & (df["label"] == "synthetic")]
            test_synth_sample = test_synth.sample(per_model_size)
            sample_list.extend([test_synth_sample])
    ...
    overall_precision, overall_recall, overall_f1, _ = precision_recall_fscore_support(y_test, y_pred,
                                                                                       average='weighted')
    ...
    misclassified = misclassified.join(sample_df[['id']], how='left')
    ...
    file_path = f"{test_dataset}/{train_dataset}_train/overall/{test_dataset}_overall_random_forest_final.csv"
    ...
    run_info = {"user": "Zeina Daboul",
                ...
                "training_models": sample_df['model'].unique(),
                "testing_models": sample_df['model'].unique(),
                "training_domains": sample_df["domain"].unique(),
                "testing_domains": sample_df["domain"].unique(),
                "accuracy": accuracy,
    ...
    run_info_df.to_csv(f'{test_dataset}/{train_dataset}_train/{test_dataset}_overall_classifier_log.csv', mode='a',
