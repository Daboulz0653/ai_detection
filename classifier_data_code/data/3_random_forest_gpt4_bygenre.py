...
for domain in df["domain"].unique():
    ...
    for i in range(100):
        ...
        auth_df = df[(df["domain"] == domain) & (df["label"] == "authentic")]
        auth_sample_size = min(len(auth_df), 1000 * len(domain_models))
        # auth_sample_size = min(len(auth_df), 1000)
        auth_sample = auth_df.sample(auth_sample_size)
        sample_list.extend([auth_sample])

        per_model_sample_size = auth_sample_size/len(domain_models)
        for model_name in domain_models:
            synth_df = df[(df["domain"] == domain) & (df["model"] == model_name) & (df["label"] == "synthetic")]
            synth_sample = synth_df.sample(round(per_model_sample_size))
            sample_list.extend([synth_sample])
        ...
        file_path = f"{test_dataset}/{train_dataset}_train/bygenre/{test_dataset}_{domain}_random_forest_final.csv"
        ...
        run_info = { "user": "Zeina Daboul",
                    ...
                    "training_models": domain_models,
                    "testing_models": domain_models,
        ...
        run_info_df.to_csv(f'{test_dataset}/{test_dataset}_bygenre_classifier_log.csv', mode='a', index=False, header=False)
