...
df = pd.read_csv(f"{test_dataset}_final_feature_matrix.csv")
...
for i in range(100):
    ...
    for domain in df["domain"].unique():
        ...
        size = min(len(test_auth), 333)
        test_auth_sample = test_auth.sample(size)
        sample_list.extend([test_auth_sample])


        test_synth = df[(df["domain"] == domain) & (df["label"] == "synthetic")]
        test_synth_sample = test_synth.sample(333)
    ...
    file_path = f"{train_dataset}_train/overall/{test_dataset}_overall_random_forest_final.csv"
    ...
    run_info = {"user": "Zeina Daboul",
                ...
                "training_models": "gpt-3.5-turbo",
                "testing_models": "gpt-3.5-turbo",
    ...
    run_info_df.to_csv('../ALL_classifier_log.csv', mode='a', index=False, header=False)
    run_info_df.to_csv(f'{train_dataset}_train/{test_dataset}_overall_classifier_log.csv', mode='a',
