...
for i in range(100):
    ...
    for domain in test_df["domain"].unique():
        ...
        test_synth = test_df[(test_df["domain"] == domain) &  (test_df["label"] == "synthetic")]
        test_synth_sample = test_synth.sample(per_domain_sample_size)
    ...
    file_path = f"HDSR_train/overall/{test_dataset}_overall_random_forest_final.csv"
    ...
    run_info_df.to_csv(f'HDSR_train/{test_dataset}_overall_HDSR_train_classifier_log.csv', mode='a', index=False, header=False)
