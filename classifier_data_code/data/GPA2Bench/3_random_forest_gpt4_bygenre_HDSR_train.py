...
test_df = pd.read_csv(f"{test_dataset}_final_feature_matrix.csv")
...
for domain in test_df["domain"].unique():
    ...
    for i in range(100):
        ...
        test_auth_sample = test_auth_df.sample(200)
        test.extend([test_auth_sample])

        test_synth_df = test_df[(test_df["domain"] == domain) & (test_df["label"] == "synthetic")]

        test_synth_sample = test_synth_df.sample(200)
        ...
        file_path = f"{train_dataset}_train/bygenre_1000/{test_dataset}_{domain}_random_forest_final.csv"
        ...
        run_info = { "user": "Zeina Daboul",
                    ...
                    "training_models": ["gpt4", "gpt-3.5-turbo"],
        ...
        run_info_df.to_csv(f'{train_dataset}_train/{test_dataset}_bygenre_1000_classifier_log.csv', mode='a', index=False, header=False)
